...
from sequence_run_manager.serializers.sample_sheet import (
...
class SequenceViewSet(GenericViewSet):
    ...
    @extend_schema(
        responses=SequenceRunSerializer(many=True),
        description="Get all sequence data by instrument run id",
    )
    @action(
        detail=False,
        methods=["get"],
        url_name="sequence_run_by_instrument_run_id",
        url_path="sequence_run",
    )
    ...
    @extend_schema(
        ...
        description="Get all states by instrument run id",
    )
    @action(
        detail=False,
        methods=["get"],
        url_name="states_by_instrument_run_id",
        url_path="states",
    )
    ...
    @extend_schema(
        ...
        description="States transition validation map for state creation, update",
    )
    @action(
        detail=False,
        methods=["get"],
    ...
    def get_states_transition_validation_map(self, request, **kwargs):
        ...
        """
        return Response(
            StateViewSet.states_transition_validation_map, status=status.HTTP_200_OK
        )

    @extend_schema(
        responses=CommentSerializer(many=True),
        description="Get all comments by instrument run id",
    )
    @action(
        detail=False,
        methods=["get"],
        url_name="comments_by_instrument_run_id",
        url_path="comments",
    )
    def comments_by_instrument_run_id(self, request, *args, **kwargs):
        ...
        """
        instrument_run_id = kwargs.get("instrument_run_id")
        sequences_orcabus_ids = Sequence.objects.filter(
    ...
    @extend_schema(
        responses=SampleSheetWithCommentSerializer(many=True),
        description="Get all sample sheets by instrument run id",
    )
    @action(
        detail=False,
        methods=["get"],
        url_name="sample_sheets_by_instrument_run_id",
        url_path="sample_sheets",
    )
    def sample_sheets_by_instrument_run_id(self, request, *args, **kwargs):
        ...
        for sample_sheet in sample_sheets:
            sample_sheet.comment = comments.filter(
